...
Sub ClearTable(SQLite As SQL,TableName As String)
	SQLite.ExecNonQuery("DELETE FROM [" & TableName & "]")
End Sub
...
public Sub GetField(SQL As SQL, TableName As String, PrimField As String, PrimValue As String, ReturnField As String) As String
	Dim mr As Map = ExecuteMap(SQL, "SELECT [" & ReturnField & "] From [" & TableName & "] WHERE [" & PrimField & "] = ?", Array As String(PrimValue))
	If mr.IsInitialized = False Then
		Return ""
	Else
		Return mr.Get(ReturnField) 
	End If
End Sub
