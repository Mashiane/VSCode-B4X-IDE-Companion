...
(async function main(){
  try {
    const installThemes = process.argv[2] || 'C:\\Program Files\\Anywhere Software\\B4A\\Themes';
    const themeName = process.argv[3] || 'Dark.vssettings';
    const filePath = path.join(installThemes, themeName);
    ...
    const themeBaseName = path.basename(filePath, path.extname(filePath));
    const theme = {
      name: `B4X Imported Theme (${themeBaseName})`,
      type: 'dark',
      colors: themeColors,
      tokenColors,
    };
    const wf = process.cwd();
    const outDir = path.join(wf, '.vscode');
    if (!fs.existsSync(outDir)) fs.mkdirSync(outDir);
    const outPath = path.join(outDir, `b4x-imported-theme-${themeBaseName}.json`);
