...
function splitTopLevelCommaSegments(source) {
    const parts = [];
    let current = '';
    let depth = 0;
...
function parseWorkspaceModule(content, fileName) {
    ...
    for (let i = 0; i < startLine; i += 1) {
        ...
        if (/^Type\s*=\s*Class$/i.test(h))
            isClassModule = true;
        if (/^Type\s*=\s*StaticCode$/i.test(h))
            isStaticModule = true;
        if (/^#StartAtBoot:/i.test(h))
    ...
    for (let i = startLine; i < lines.length; i += 1) {
        ...
        if (globalsSub && new RegExp(`^Sub\\s+${globalsSub}\\b`, 'i').test(code))
            inGlobalsBlock = true;
        if (/^End\s+Sub\b/i.test(code) && inGlobalsBlock)
    ...
    for (let lineNumber = startLine; lineNumber < lines.length; lineNumber += 1) {
        ...
        if (methodName && !/^Class_Globals$/i.test(methodName) && !/^Process_Globals$/i.test(methodName)) {
            ...
            const parameters = parseTypedNameList(methodMatch.groups?.parameters ?? '').map((d, idx) => ({
    ...
    const moduleType = isClassModule && !isStaticModule ? 'class' : isStaticModule && !isClassModule ? 'static' : isServiceModule ? 'service' : 'unknown';
...
async function extractFromZip(archivePath) {
    ...
    for (const archivePath of b4xlibFiles) {
        ...
        try {
            ...
            for (const mod of modules) {
                if (!mod.name)
                    errors.push(`Module has no name`);
                if (mod.moduleType === 'unknown')
                    errors.push(`Module '${mod.name}' has unknown type (no Type= header and no service lifecycle methods)`);
                if (mod.methods.some(m => !m.name))
                    errors.push(`Module '${mod.name}' has method with no name`);
                if (mod.properties.some(p => !p.name))
                    errors.push(`Module '${mod.name}' has property with no name`);
            }
            const stats = {
