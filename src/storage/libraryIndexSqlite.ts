  ...
  prepare(sql: string) {
    ...
    if (t.startsWith('INSERT INTO B4XLIBS')) {
      return { run(...args: any[]) {
        ...
        if (existing) { existing.mtime = mtime; existing.extractedAt = extractedAt; existing.extractedDir = extractedDir; existing.manifestBlob = manifestBlob; }
        else s.tables.b4xlibs.push({ id: s.idCounters.b4xlibs++, archivePath, mtime, extractedAt, extractedDir, manifestBlob });
        return { changes: 1 };
      } };
    }
...
export interface ParsedModuleBlob {
  moduleKind: 'class' | 'static' | 'service';
  name: string;
  methods: any[];
  properties: any[];
  doc?: string;
...
class LibraryIndex {
  ...
  public async init(storageBase?: string): Promise<void> {
    if (this.db) return;
  ...
  public upsertParsedForPath(absPath: string, mtime: number, size: number, parsed: ParsedModuleBlob): void {
    ...
    try {
      ...
      // Fallback for SQLite builds that may not support UPSERT syntax: delete and insert.
      try {
        this.db.prepare('DELETE FROM files WHERE absPath = ?').run(absPath);
        this.db.prepare('INSERT INTO files(absPath, mtime, size, parsedBlob, lastParsed, lastSeen) VALUES(?, ?, ?, ?, ?, ?)').run(absPath, mtime, size, blob, now, now);
      } catch { /* ignore */ }
    }
  }
  ...
  public upsertXmlClasses(xmlPath: string, classes: { name: string; methods: any[]; properties: any[]; doc?: string }[]): void {
    ...
    try {
      for (const cls of classes) {
        try { insert.run(xmlPath, cls.name, JSON.stringify(cls)); } catch { /* ignore per-row */ }
      }
    } catch { /* ignore */ }
  }
  ...
  public upsertB4xlibArchive(
    archivePath: string,
    mtime: number,
    extractedDir: string,
    innerFiles: { relPath: string; absPath: string; mtime: number; size: number }[],
    ...
    const up = this.db.prepare(
      'INSERT INTO b4xlibs(archivePath, mtime, extractedAt, extractedDir, manifestBlob) VALUES(?, ?, ?, ?, ?) ON CONFLICT(archivePath) DO UPDATE SET mtime = ?, extractedAt = ?, extractedDir = ?, manifestBlob = ?',
    );
    up.run(archivePath, mtime, now, extractedDir, manifestJson, mtime, now, extractedDir, manifestJson);
  ...
  public getB4xlibArchive(archivePath: string): { mtime: number; extractedDir: string; manifest?: Record<string, string> } | null {
    if (!this.db) return null;
    const row = this.db.prepare('SELECT mtime, extractedDir, manifestBlob FROM b4xlibs WHERE archivePath = ?').get(archivePath);
    if (!row) return null;
    let manifest: Record<string, string> | undefined;
    if (row.manifestBlob) {
      try { manifest = JSON.parse(row.manifestBlob); } catch { /* ignore */ }
    }
    return { mtime: row.mtime ?? 0, extractedDir: row.extractedDir ?? '', manifest };
  }
